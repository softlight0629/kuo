import React, { Component } from 'react';
import { observer } from 'mobx-react';

@observer
class SlideShowGalleryLayout extends Component {

  render() {
    const { galleryMedias } = this.props.astm.store;

    return (
      <div className="slideshow-layout">
        <div className="gallery-container one-row hide-scrollbar" style={{ width: 645, height: 362 }}>
          <div className="gallery-column" style={{ width: 645 }}>
            <div className="gallery-group gallery-group-visible" style={{ width: 645, height: 362 }}>
              <div className="gallery-item-container visible" style={{ width: 645, margin: 0, top: 0, left: 0 }}>
                <div className="gallery-item-wrapper visible" style={{ height: 362 }}>
                  <div className="gallery-item-visible gallery-item">
                    <img className="gallery-item gallery-item-visible" src={galleryMedias[0].cover}/>
                  </div>
                  <div className="gallery-item-info gallery-item-bottom-info">
                    <div className="gallery-item-social gallery-item-direction-center"></div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <button className="nav-arrows-container next">
            <img src="data:image/svg+xml;base64," />
          </button>
        </div>
      </div>
    )
  }
}

export default SlideShowGalleryLayout;

...
class ItemRow extends Component {
  ...
  duplicate = (radioBtn) => {
    ...
    store.duplicateRadioBtn(radioBtn);
  }

  delete = (radioBtn) => {
    const { store } = this.props;
    store.removeRadioBtn(radioBtn);
  }

  setAsDefault = (radioBtn) => {
  }

  done = radioBtn => {
    if (this.state.editMode === 'label') {
      radioBtn.setLabel(this.state.value);
    }

    if (this.state.editMode === 'value') {
      radioBtn.setValue(this.state.value);
    }
  ...
  renderOptionMenu = (radioBtn) => {
    return (
      <div className="dropdown-options-menu">
        <Menu>
          <Menu.Item key="0">
            <div className="option-menu-action" onClick={() => { this.setAsDefault(radioBtn) }}>
              <span className="symbol ">
                <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                  <title>Set Selected</title>
                  <g fill="#2b5672" fillRule="evenodd">
                    <path d="M10 6c-2.206 0-4 1.794-4 4s1.794 4 4 4 4-1.794 4-4-1.794-4-4-4m0 1a3 3 0 1 1 0 6 3 3 0 0 1 0-6"></path>
                    <path d="M10 3a7 7 0 1 0 0 14 7 7 0 0 0 0-14m0 1c3.309 0 6 2.691 6 6s-2.691 6-6 6-6-2.691-6-6 2.691-6 6-6"></path>
                  </g>
                </svg>
              </span>
          ...
          <Menu.Item key="1">
            <div className="option-menu-action" onClick={() => { this.duplicate(radioBtn) }}>
              <span className="symbol ">
                <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <Menu.Item key="2">
            <div className="option-menu-action" onClick={() => { this.editLabel(radioBtn) }}>
              <span className="symbol ">
                <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <Menu.Item key="3">
            <div className="option-menu-action" onClick={() => { this.editValue(radioBtn) }}>
              <span className="symbol ">
                <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <Menu.Item key="4">
            <div className="option-menu-action" onClick={() => { this.delete(radioBtn) }}>
              <span className="symbol ">
                <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
  ...
  renderViewMode = radioBtn => (
    <div className="view-mode-content">
      <div className="text-container">
        <div className="text-inner-container">
          <span className="has-tooltip">
            <div className="tooltip-on-ellipsis-content">
              <div className="label">{radioBtn.label}</div>
            </div>
          </span>
          <span className="has-tooltip">
            <div className="tooltip-on-ellipsis-content">
              <div className="value">{radioBtn.value}</div>
            </div>
          </span>
        </div>
      </div>
      <div className="context-menu-container">
        <div className="context-menu">
          <Dropdown overlay={this.renderOptionMenu(radioBtn)} trigger={['click']}>
  ...
  renderEditMode = (radioBtn) => {
    const prop = this.state.editMode;
    const value = prop === 'label' ? radioBtn.label : radioBtn.value;
        ...
        <span className="has-tooltip">
          <div className="input-container">
            <Input 
              ...
              spellCheck="false"
              dir="ltr"
              onKeyDown={(e) => this.onKeyDown(e, radioBtn)}
              onChange={(e) => this.handleChange(e)}
            />
          </div>
        </span>
        <Button onClick={() => this.done(radioBtn)}>Done</Button>
      </div>
    )
  }

  render() {
    const { radioBtn } = this.props;
      ...
      <div className={`sbdl-item-row ${editMode?'edit-mode': ''}`}>
        <div className="sbdl-item-row-base">
          ...
          <div className={`content ${editMode?'': 'has-drag-handle'}`}>
            { !editMode && this.renderViewMode(radioBtn) }
            { editMode && this.renderEditMode(radioBtn) }
          </div>
        </div>
      </div>
    )

...
class GalleryContainer extends Component {
  ...
  componentWillReceiveProps(newProps) {
    try {
      ...
      if (newProps.items) {
        let canSetItems =
          newProps.items.length > this.state.items.length
          || _.includes(['manuallySetImages', 'albumSetFromBuilder'], newProps.galleryDataSrc)
          || utils.isPlayground();

        if (canSetItems) {
          this.items = newProps.items;
          this.reRender(this.renderTriggers.ALL);
        } else if (newProps.items.length < this.state.items.length) {
          console.watn('Got new props with fewer items', this.newProps, this.state.items);
        }
      }
      if (newProps.styles && !_.isEqual(this.props.styles, newProps.styles)) {
        this.reRender(this.this.renderTriggers.STYLES);
      }
      ...
      console.log('Failed settings new props', e);
    }
  }
  ...
  initEventsFunction() {
    ...
    this.eventsFunctions.push([consts.Events.STYLE_PARAMS_CHANGE, this.reRenderForStyles.bind(this)]);
    this.eventsFunctions.push([consts.Events.SETTINGS_UPDATED, this.reRenderForSettings.bind(this)]);
    this.eventsFunctions.push([consts.Events.EDIT_MODE_CHANGE, this.reRenderForEditMode.bind(this)]);
    this.eventsFunctions.push([consts.Events.SCROLL, this.reRenderForScroll.bind(this)]);
    this.eventsFunctions.push([consts.Events.RESIZE, this.onResizeEvent.bind(this)]);
  }

  initEventListeners() {
    this.eventsFunctions.forEach(x => {
      this.props.events.on(x[0], x[1]);
    });
  }
  ...
  getStyleBySeed(seed) {
    ...
    var style = {
      ...
      collageDensity: numFromSeed(1, 100, 'collageDensity') / 100,
      groupTypes: ['1'].concat(_.filter('2h,2v,3t,3b,3l,3r,3h,3v'.split(','), function (type, i) {
        return boolFromSeed('groupTypes' + i);
      })),
      oneRow: boolFromSeed('oneRow'),
      borderRadius: 0,
      boxShadow: 0,
      imageMargin: numFromSeed(0,  numFromSeed(300, 800, 'gallerySize') / 5, 'imageMargin'),
      galleryMargin: 0,
      floatingImages: 0,
  ...
  getStyleParamsState() {
    ...
    var sp = this.props.getStyleParams();
  ...
  getVisibleBounds(scrollTop, pageScale) {
    ...
    if (this.state.styleParams.oneRow) {
      ...
      screenSize = utils.getScreenWidth() / pageScale;
